...
function generateImage(langs) {
  ...
  let bgColor = "#1a1b27";
  let borderColor = "#fff";
  let titleColor = "#6fa4fc";
  let textColor = "#39bdae";

  /* generate an image with a heading "Languages" and a list including all langs*/
  ...
  let writeY = 80;
  langs.forEach((lang) => {
    let langColor = colors[lang] || { color: textColor };
    ctx.fillStyle = langColor.color;
    ctx.fillText("•", 20, writeY);

    ctx.fillStyle = textColor;
    ctx.fillText(lang, 40, writeY);
    writeY += 30;
  });
